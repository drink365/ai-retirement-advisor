...
def calculate_retirement_cashflow(
    ...
    計算退休現金流，考慮:
      ...
      - 在尚未買房前，住房費用 = 租金；買房當年起不再付租金，改付頭期 + 房貸
      - 投資報酬、通膨影響
      - 預期壽命計算到最後一年
      - 一次性支出 (不考慮通膨) 在當年直接扣除 lumpsum

    重要邏輯：
      if mortgage_year < 0:  # 還沒到買房年齡
          housing_expense = 租金
      if mortgage_year == 0: # 買房當年
          housing_expense = 頭期款 + 當年房貸
      if 0 < mortgage_year < loan_term: # 貸款期
          housing_expense = 房貸
      if mortgage_year >= loan_term:
          housing_expense = 0 (房貸繳完)
    """
    ...
    for i, age in enumerate(ages):
        ...
        if rent_or_buy == "租房":
            ...
            if mortgage_year < 0:
                # 還沒買房 => 付租金  (★ 關鍵：確保這裡是 rent_amount * 12)
                housing_expense = int(rent_amount * 12)
