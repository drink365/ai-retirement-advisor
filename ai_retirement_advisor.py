...
def calculate_retirement_cashflow(
    ...
    計算退休現金流:
      - 收入: 薪資、投資收益、退休年金
      - 支出: 生活費(含通膨) + 住房費 + 一次性支出(不含通膨)
      - 買房前/買房當年/貸款期/貸款滿等邏輯
      - lumpsum_list 為 list of dict, 每筆 {年齡, 金額}

    注意:
      - 僅計算年齡 >= current_age、金額>0 的一次性支出
    """
...
st.write(f"年齡需 >= {current_age} 且金額>0 才會生效")
...
if len(st.session_state["lumpsum_list"])==0:
    ...
    for i, row_data in enumerate(st.session_state["lumpsum_list"]):
        with st.expander(f"年齡={row_data['年齡']}, 金額={row_data['金額']}"):
            ...
            # 刪除
            if col1.button(f"刪除此支出", key=f"del_{i}"):
                st.session_state["lumpsum_list"].pop(i)
                st.experimental_rerun()

            # 編輯
            edit_age = col2.number_input(f"編輯年齡_{i}", min_value=0, value=row_data["年齡"])
            edit_amt = col2.number_input(f"編輯金額_{i}", min_value=0, value=row_data["金額"])
            if col2.button(f"確定更新_{i}"):
                if edit_age>=current_age and edit_amt>0:
                    st.session_state["lumpsum_list"][i] = {"年齡":edit_age, "金額":edit_amt}
                    st.success("編輯成功")
                else:
                    st.warning("不符合條件(年齡<current_age or 金額<=0) => 會被跳過")
                st.experimental_rerun()
