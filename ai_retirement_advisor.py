...
st.header("👋 您好，我是您的退休助手 Nana")
st.markdown("歡迎使用我們的退休規劃平台，我們將竭誠為您提供專業且貼心的服務。")

# ─────────────────────────
# 一、基本資料輸入區
# ─────────────────────────
st.subheader("📝 基本資料")
st.info("請輸入您的基本資料，以便我們根據您的狀況提供合適的退休規劃建議。")
col1, col2 = st.columns(2)
with col1:
    current_age = st.number_input("您的年齡", min_value=18, max_value=100, value=40)
...
# ─────────────────────────
st.subheader("🏠 住房狀況")
st.info("請選擇您的住房狀況，這將幫助我們更全面地規劃您的退休藍圖。")
...
st.info("若您預計有特殊或一次性支出，請在此處登記，我們將納入整體規劃中。")
...
st.success("計算完成，以上為您的退休現金流預估結果。")

# ─────────────────────────
# 五、退休風格測驗與智能建議報告
# ─────────────────────────
st.subheader("🎯 退休風格測驗與建議報告")
st.info("請選擇您理想中的退休生活風格，我們將根據您的回答提供參考建議。")
retire_style = st.radio("您的理想退休生活？", ["低調簡約", "舒適中產", "高端奢華"], key="retire_style")
...
st.markdown(f"根據您的選擇，建議的退休目標資產約為 **{recommended_target:,.0f}** 元。")
...
if len(retire_idx) > 0:
    ...
    st.markdown(f"在您設定的退休年齡 **{retirement_age}** 歲時，預計累積資產約為 **{proj_asset:,.0f}** 元。")
    st.markdown(f"與您的目標 **{target_asset:,.0f}** 元相比，尚有 **{gap:,.0f}** 元的差距。")
    ...
    if gap > 0:
        st.markdown("**建議：**")
        st.markdown("• 您目前的儲蓄與投資計劃可能尚需調整，建議考慮延後退休、增加儲蓄或優化投資組合。")
        st.markdown("• 若需要進一步的專業建議，請考慮預約免費的財務規劃諮詢。")
        ...
        st.markdown("看起來您的退休規劃已十分完善，請持續關注您的財務狀況。")
else:
...
st.info("下列圖表顯示不同通膨情境下的累積結餘走勢，供您參考。")
...
# 八、行銷資訊
# ─────────────────────────
st.markdown("如需進一步協助，請造訪 [永傳家族辦公室](https://www.gracefo.com) 或直接聯繫我們，我們將竭誠為您服務。")
